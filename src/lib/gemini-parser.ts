...
1. **اسم الفستان (name)**: استخرج السطر الأول أو نوع القماش وتفاصيله بدون إيموجيات (مثال: "قماش ستان نخب اول مع شال طويل").
...
4. **الألوان والمقاسات (variants)**:
   - لكل لون مذكور: استخرج اسم اللون الصافي بالعربية (مثل: "اسود", "ابيض", "خمري", "زهري", "اصفر", "بيبي بلو", "بني", "كحلي", "زيتي", "سومو", "عنابي", "نهدي", "اوف وايت").
   - حدد كود الهاكس (colorHex) المناسب للون (مثل: اصفر=#EAB308, خمري=#722F37, اسود=#000000, ابيض=#FFFFFF, زهري=#F472B6, بيبي بلو=#89CFF0, بني=#78350F, كحلي=#1E3A5F, زيتي=#4D7C0F, سومو=#F87171, عنابي=#831843, نهدي=#9333EA, اوف وايت=#FAF7F2).
   ...
   - إذا ذُكر "خالص" بدون أي مقاس، أو لم تُذكر مقاسات بجانبه، لا تضف له مقاسات موجبة.
   - إذا ذُكر "خالص ما عدا 38" أضف فقط المقاس "38" بكمية 5.
   - الكمية (quantity): لكل مقاس متوفر ضع 5، وللمقاسات غير المتوفرة ضع 0.
...
const MODELS = ['gemini-3.5-flash', 'gemini-3.7-flash', 'gemini-3.6-flash', 'gemini-3.1-flash-lite'];
...
export async function parseDressWithGemini(postText: string): Promise<ParsedDressAI | null> {
  ...
  for (const modelName of MODELS) {
    try {
      ...
      console.warn(`⚠️ تعذر استخدام ${modelName}، جاري تجربة الموديل البديل...`);
    }
  }
